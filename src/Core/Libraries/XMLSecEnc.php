...
class XMLSecEnc {
   ...
   <xenc:CipherData>
    ...
    public function addReference($name, $node, $type) {
        ...
        $refuri = \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityDSig::generate_GUID();
    ...
    public function encryptNode($objKey, $replace=TRUE) {
        ...
        if (!$objKey instanceof \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey) {
     ...
     *
     * @params Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey $objKey  The decryption key that should be used when decrypting the node.
    ...
    public function encryptKey($srcKey, $rawKey, $append=TRUE) {
        if ((!$srcKey instanceof \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey) || (!$rawKey instanceof \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey)) {
            throw new Exception('Invalid Key');
        }
    ...
    public function locateKey($node=NULL) {
        ...
        if ($doc = $node->ownerDocument) {
            ...
            if ($encmeth = $nodeset->item(0)) {
                ...
                try {
                    $objKey = new \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey($attrAlgorithm, array('type' => 'private'));
                } catch (Exception $e) {
                    return NULL;
                }
                return $objKey;
    ...
    static function staticLocateKeyInfo($objBaseKey=NULL, $node=NULL) {
        ...
        $xpath = new DOMXPath($doc);
        $xpath->registerNamespace('xmlsecenc', XMLSecEnc::XMLENCNS);
        $xpath->registerNamespace('xmlsecdsig', \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityDSig::XMLDSIGNS);
        ...
        foreach ($encmeth->childNodes AS $child) {
            switch ($child->localName) {
                    ...
                    foreach ($child->childNodes AS $keyval) {
                        switch ($keyval->localName) {
                            ...
                            case 'RSAKeyValue':
                                ...
                                $publicKey = \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey::convertRSA($modulus, $exponent);
                ...
                case 'RetrievalMethod':
                    ...
                    return \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey::fromEncryptedKeyElement($keyElement);
                case 'EncryptedKey':
                    return \Pieterf\LaravelIngIdealAdvanced\Core\Libraries\XMLSecurityKey::fromEncryptedKeyElement($child);
                case 'X509Data':
